function [coeff_map, accuracy_map] = band_regression(structure,key,step,freq_lim,num_cv,compare,logtransform);
...
if compare == 1
    accuracies = [];
    if logtransform == 1
        [feature_table,~] = freq_power_analysis(structure,1);
    else
        [feature_table,~] = freq_power_analysis(structure,0);
    end
    for i = 1:10 
        cv = cvpartition(size(feature_table,1),'HoldOut',0.3);
        idx = cv.test;
