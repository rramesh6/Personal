function [coeff_map, accuracy_map] = band_lasso(structure,key,step,freq_lim,num_cv,compare)
...
for bs = num_bands
    ...
    for i = 1:num_cv
        ...
        [B, stats] = lasso(dataTrain(:,1:bs),dataTrain(:,end));
        [~, minErrorIdx] = min(stats.MSE);
        temp_coeffs = B(:,minErrorIdx)';
        coeff_chunks = [coeff_chunks; temp_coeffs];

        y_hat = round(sum(temp_coeffs .* dataTest(:,1:bs),2) + stats.Intercept(minErrorIdx));
...
xlabel('Frequency (Hz)')
ylabel('Number of Bands')
title(['RCS02 Band Search - LASSO - Key ' num2str(key)])
frequencies = cellfun(@num2str, num2cell(key_struct.F(1,1:freq_lim*10+1)), 'UniformOutput', false);
frequencies = string(frequencies);
frequencies(mod(key_struct.F(1,1:freq_lim*10+1),5) ~= 0) = " ";
h.XDisplayLabels = frequencies;
